...
int BackendService::init(const ServiceConfig& service_config, Backend* backend) {
    ...
    if (service_config.has_request()) {
        ...
        if (request_policy_name == "default") {
            const BRPC_NAMESPACE::AdaptiveProtocolType protocol =
                    _backend->channel()->options().protocol;
            if (protocol == BRPC_NAMESPACE::PROTOCOL_HTTP && !_backend->is_dynamic()) {
                request_policy_name = "http_default";
            } else if (protocol == BRPC_NAMESPACE::PROTOCOL_HTTP && _backend->is_dynamic()) {
                request_policy_name = "dynamic_default";
    ...
    if (service_config.has_response()) {
        ...
        if (response_policy_name == "default") {
            ...
            if (protocol == BRPC_NAMESPACE::PROTOCOL_HTTP && !_backend->is_dynamic()) {
                ...
                response_policy_name = "dynamic_default";
            } else if (protocol == BRPC_NAMESPACE::PROTOCOL_REDIS) {
