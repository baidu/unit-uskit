...
int DynamicHttpRequestPolicy::run(BackendController* cntl) const {
    ...
    for (auto& dynamic_ele : dynamic_args->GetObject()) {
        ...
        for (size_t index = 0; index < dynamic_ele.value.GetArray().Size(); ++index) {
            ...
            //   channel->CallMethod(
            //            nullptr, &brpc_cntl, nullptr, nullptr, BRPC_NAMESPACE::DoNothing());
            //} else if (!cntl->get_recall_next().empty()) {
            //    channel->CallMethod(nullptr, &brpc_cntl, nullptr, nullptr,
            //    cntl->_jump_done.get());
            //} else if (
            ...
            //            nullptr, &brpc_cntl, nullptr, nullptr, BRPC_NAMESPACE::DoNothing());
            //    US_LOG(WARNING)
            //            << "Fail to run cancel policy, no correct order given, ignored as NONE";
            //}
            channel->CallMethod(
                    nullptr, brpc_cntl.get(), nullptr, nullptr, BRPC_NAMESPACE::DoNothing());
...
int DynamicHttpRequestPolicy::run(
    ...
    for (auto& dynamic_ele : dynamic_args->GetObject()) {
        ...
        for (size_t index = 0; index < dynamic_ele.value.GetArray().Size(); ++index) {
            ...
            BRPC_NAMESPACE::Channel* channel = _backend->channel();
            channel->CallMethod(nullptr, brpc_cntl.get(), nullptr, nullptr, cntl->_jump_done.get());
            dyn_http_cntl->brpc_controller_list().push_back(std::move(brpc_cntl));
        }
        break;
    }
